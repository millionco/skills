...
budge is a self-contained IIFE loaded from \`https://budge.design/budge.iife.js\` via \`next/script\`. On local/dev hosts, it watches HMR-driven \`class\` and \`style\` mutations, infers the latest numeric visual property change, and mounts automatically. It also uses React Grab primitives for manual selection: press \`Cmd+Shift+S\`, select an element, and budge opens. It also supports a \`data-budge\` config element as a fallback. It handles live preview on \`[data-budge-target]\` elements, arrow key stepping, slide navigation, audio feedback, submit, and cancel entirely client-side.
